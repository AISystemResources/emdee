...
11. **New or modified MCP tools require a Playwright e2e spec in the same PR.** Any change to a tool's request shape, response shape, or observable side effects must ship with an `e2e/mcp/*.spec.ts` that exercises the tool — either via `POST /api/mcp` (when test OAuth is available) OR by calling the tool function directly against a local-mode `ToolContext` + temp filesystem (SPRINT-054 pattern, see `e2e/mcp/move_doc.spec.ts`). Both paths run the same tool code with the same vault helpers, so the safety promise is preserved either way. CI's `playwright (chromium)` check then gates the runtime correctness — without this, a syntactically valid but runtime-broken tool can slip through all four checks and reach prod. Pure refactors (no behaviour change) are exempt but a regression-touching spec is encouraged. (Source: Ralph-loop SPRINT-048, 2026-06-23 — closes the "all checks green, tool broken at first call" failure mode. Clarified by SPRINT-054 to accept local-mode specs given the OAuth-bypass gap.)
12. **PRs touching `supabase/migrations/**` auto-merge like any other PR, gated on the required `check-drift` status check.** Since 2026-07-27 the `check-drift` job is a required check on `main` (via GitHub branch protection). It fails any PR where a repo migration file isn't applied to BOTH prod and test Supabase projects. Because that catches the highest-blast-radius failure mode automatically, migration PRs no longer need special human-review carve-out — arm auto-merge on them the same way as any other PR (`gh pr merge <n> --auto --squash`). Corollary: whoever writes a migration is responsible for applying it via `apply_migration` (or `supabase db push`) to both projects BEFORE opening the PR — otherwise `check-drift` blocks the merge. (Source: replaces the 2026-06-23 Ralph-loop SPRINT-048 carve-out. Original rule required manual human merge because check-drift was optional; making it required removes the need.)

### Deploy ceiling — feat/* model (autonomous PRs land on main directly via CI gate)

**All work — human or autonomous — lands on `main` via `feat/<sprint-id>-<slug>` PRs gated by 4 required CI checks.** No staging tier. The autonomous Ralph loop opens PRs directly to `main`; auto-merge fires on green CI. Cognitive load stays low (one branch to think about); safety stays acceptable because (a) HARD RULES 11/12 gate the catastrophic failure modes, (b) all other regressions are 5-minute revert problems.
...
- **Preflight before every new feat branch:** `git fetch origin && git checkout main && git pull --ff-only && git checkout -b feat/SPRINT-NNN-<slug>`. Always branch from the latest `origin/main`, never from a stale local main. When multiple PRs are open simultaneously and one merges, the others go `BEHIND` and auto-merge silently stalls. The `.github/workflows/auto-update-prs.yml` workflow fires on every push to main and calls `gh pr update-branch` on any BEHIND PR — the queue chain-reacts through automatically. Manual "Update branch" clicks are unnecessary; if a PR sits BEHIND for more than a few minutes, check that workflow's run log.
- **Open a PR back to `main` when the sprint is shippable.** The PR is the ceiling — no merge without human review.
- **After opening a PR, always enable auto-merge:** `gh pr merge <n> --auto --squash`. The PR merges automatically the moment all 4 required CI checks go green. Do this every PR without exception.
- **No direct pushes to `main`**, attended or not — not even for one-liner fixes. Every change goes through a `feat/*` branch + PR + 4 green CI checks, full stop. `enforce_admins: false` means the repo owner _can_ bypass in an emergency, but that path is never the default. If branch protection blocks something the user explicitly wants merged, surface it; don't try to circumvent.
