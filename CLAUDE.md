...
- **Preflight before every new feat branch:** `git fetch origin && git checkout main && git pull --ff-only && git checkout -b feat/SPRINT-NNN-<slug>`. Always branch from the latest `origin/main`, never from a stale local main. When multiple PRs are open simultaneously, GitHub will show "out-of-date with base branch" on the younger PRs — UI noise only (branch protection has `strict:false`); auto-merge still fires on the 4 green checks. Don't manually click "Update branch."
