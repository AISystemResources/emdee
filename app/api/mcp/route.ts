...
- Paths starting with "__shared__/<owner_id>/" are docs another user has
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      { name: "lint_vault_autofix", description: "SPRINT-102 vault hygiene auto-fix. Tier 1: strip redundant `## Associated with` bullets flagged by sibling_assoc_redundant / associate_duplicates_hierarchy. Tier 1.5: repair one-parent invariant by keeping first `## Child of` bullet as canonical and demoting the rest to `## Associated with` (multiple_child_of). Every fix is mechanical + idempotent + preserves both relationships. Defaults to dry-run (returns the plan without writing); pass `dry_run: false` to apply. Returns { tier, dry_run, scanned, planned_fixes, docs_to_modify, bullets_to_remove, parents_to_demote, applied, failed?, remaining_warnings_estimate }. Asymmetric-edge auto-repair (Tier 2) deferred.", inputSchema: { type: "object", additionalProperties: false, properties: { dry_run: { type: "boolean", description: "Default true. Pass false to actually write the fixes." } } } },
