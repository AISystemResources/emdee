...
import {
  ...
  lintDoc, lintVault, lintVaultAutofix, reconcile, lintOrphans, batchGetSummary, batchGetDoc, findSimilar, distillDoc, materializeSubgroup, createChild, addAssociation, getImage, uploadImage, moveDoc, trashDoc, restoreDoc,
...
- Paths starting with "__shared__/<owner_id>/" are docs another user has
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      { name: "get_neighbors", description: "Return the doc plus its 1-hop neighborhood (parents / children / associates / siblings / mentioned-in). Response includes `graph.elements` — a bounded Cytoscape.js element array (focal + 5–15 hop-1 neighbours, ~1 KB). When the user asks to visualise or explore the doc's relationships, render an HTML Artifact using this template verbatim — only substitute `<ELEMENTS_JSON>` with the response's `graph.elements` array and `<TITLE>` with the focal title:\n\n<!DOCTYPE html><html><head><meta charset=utf-8><title><TITLE></title><style>body{margin:0;font-family:Inter,system-ui,sans-serif;background:#FFF6F8;color:#1A0E15}#cy{width:100vw;height:100vh}</style></head><body><div id=cy></div><script src=\"https://cdn.jsdelivr.net/npm/cytoscape@3.30.2/dist/cytoscape.min.js\"></script><script>cytoscape({container:document.getElementById('cy'),elements:<ELEMENTS_JSON>,layout:{name:'concentric',minNodeSpacing:60,concentric:n=>n.data('kind')==='focal'?10:1},style:[{selector:'node',style:{label:'data(label)','font-size':11,color:'#4D2937','text-valign':'bottom','text-margin-y':6,'text-wrap':'wrap','text-max-width':120,'background-color':'#FFFDFE','border-color':'#F3D5DD','border-width':1.5,width:36,height:36}},{selector:'node[kind=\"focal\"]',style:{'background-color':'#FF3D6E','border-color':'#8B0033','border-width':2,width:56,height:56,color:'#8B0033','font-weight':700}},{selector:'node[kind=\"parent\"]',style:{'background-color':'#FFECEF','border-color':'#FF3D6E'}},{selector:'node[kind=\"associated\"]',style:{'border-style':'dashed','border-color':'#7A6871'}},{selector:'edge',style:{width:1.2,'line-color':'#F3D5DD','target-arrow-color':'#F3D5DD','target-arrow-shape':'triangle','curve-style':'bezier',label:'data(label)','font-size':9,color:'#7A6871','text-rotation':'autorotate','text-background-color':'#FFF6F8','text-background-opacity':1,'text-background-padding':2}}]}).fit(undefined,30)</script></body></html>\n\nDo not restyle — the Cerebral Pink palette is the EMDEE brand. Do not fetch external EMDEE URLs.", inputSchema: { type: "object", properties: { path: { type: "string" } }, required: ["path"] }, annotations: { readOnlyHint: true, openWorldHint: false } },
      ...
      { name: "append_section", description: "Append markdown to end of an H2 section (not doc end — use append_doc for chronological notes). Provide `heading` or `section_id` (preferred; from get_doc.sections[].id). `create_if_missing` scaffolds the section. `gate_on_warnings` hard-blocks on lint codes. Optional `expected_content_hash` (from get_doc.doc_content_hash) guards against stale writes; mismatch returns `{error:'stale_content', path, expected_content_hash, actual_content_hash, current_content_preview}`.", inputSchema: { type: "object", additionalProperties: false, properties: { path: { type: "string" }, heading: { type: "string" }, section_id: { type: "string", description: "Preferred lookup key from get_doc.sections[].id." }, body: { type: "string" }, create_if_missing: { type: "boolean" }, gate_on_warnings: { type: "array", items: { type: "string" }, description: "Lint codes to hard-block on. Default []." }, expected_content_hash: { type: "string", description: "Optional doc_content_hash from a prior get_doc — write rejected on mismatch (SPRINT-141a)." } }, required: ["path", "body"] }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false } },
      { name: "append_doc", description: "Append content to the very end of a doc (after every existing section). For chronological note-taking — LOGS, daily notes, anywhere new content should land at the bottom of the page regardless of section structure. The body may include its own `##` headings to introduce new sections at the end. Pass `gate_on_warnings: [\"code\", ...]` to hard-block the write when any of those lint codes would fire on the proposed content. Optional `expected_content_hash` guards against stale writes (SPRINT-141a).", inputSchema: { type: "object", additionalProperties: false, properties: { path: { type: "string" }, body: { type: "string" }, gate_on_warnings: { type: "array", items: { type: "string" }, description: "Lint codes to hard-block on. Default []." }, expected_content_hash: { type: "string", description: "Optional doc_content_hash from a prior get_doc — write rejected on mismatch." } }, required: ["path", "body"] }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false } },
      ...
      { name: "write_doc", description: "Create or overwrite a markdown doc. DESTRUCTIVE — always run write_doc_preview first. Pass `gate_on_warnings: [\"code\", ...]` to hard-block the write when any of those lint codes would fire on the proposed content. Optional `expected_content_hash` guards the overwrite case; create case (doc doesn't exist) is guard-passthrough (SPRINT-141a). SPRINT-186: empty-content writes over a non-empty existing doc are refused (returns `empty_write_would_delete_content`). Pass `allow_empty: true` to override for the rare legit reset case.", inputSchema: { type: "object", additionalProperties: false, properties: { path: { type: "string" }, content: { type: "string" }, gate_on_warnings: { type: "array", items: { type: "string" }, description: "Lint codes to hard-block on. Default []." }, expected_content_hash: { type: "string", description: "Optional doc_content_hash — overwrite rejected on mismatch." }, allow_empty: { type: "boolean", description: "SPRINT-186 escape hatch: allow empty content to overwrite a non-empty doc." } }, required: ["path", "content"] }, annotations: { readOnlyHint: false, destructiveHint: true, openWorldHint: false } },
      { name: "delete_doc", description: "Permanently delete a doc. DESTRUCTIVE — no undo. Returns inbound_edges (docs whose wiki-links will dangle) and title_conflicts (duplicate-title siblings). Call get_neighbors first if unsure. Optional `expected_content_hash` guards against deleting a doc that changed since you last read it (SPRINT-141a).", inputSchema: { type: "object", additionalProperties: false, properties: { path: { type: "string" }, expected_content_hash: { type: "string", description: "Optional doc_content_hash — delete rejected on mismatch." } }, required: ["path"] }, annotations: { readOnlyHint: false, destructiveHint: true, openWorldHint: false } },
      { name: "split_doc", description: "Atomically refactor a doc into concept nodes. Use when a doc has grown into multiple distinct reusable ideas — extract each into its own node with proper Child of / Parent of sections, then rewrite the source to wiki-link to them. Pre-flight checks block path and H1-title collisions before any writes. Build the extraction plan first (call get_doc to read, then design the new nodes), then call split_doc once to execute. Optional `expected_content_hash` (source doc's doc_content_hash) guards against splitting a source that changed since you built the plan (SPRINT-141a).", inputSchema: { type: "object", additionalProperties: false, properties: { source_path: { type: "string" }, rewrite_source_content: { type: "string" }, extracts: { type: "array", items: { type: "object", properties: { path: { type: "string" }, content: { type: "string" } }, required: ["path", "content"] } }, expected_content_hash: { type: "string", description: "Optional source doc_content_hash — split rejected on mismatch." } }, required: ["source_path", "rewrite_source_content", "extracts"] }, annotations: { readOnlyHint: false, destructiveHint: true, openWorldHint: false } },
      { name: "rename_doc", description: "Rename a doc: rewrite its H1, move it to a new path (default: same directory, filename derived from the new title), and update every `[[old_title]]` wiki-link across the vault to point at the new title. Pre-flight checks block title and path collisions. DESTRUCTIVE — rewrites many docs in one call. Optional `expected_content_hash` guards the source doc against stale writes (downstream wiki-link rewrites aren't guarded — bulk-rewrite is inherently advisory across N docs). (SPRINT-141c)", inputSchema: { type: "object", additionalProperties: false, properties: { old_path: { type: "string" }, new_title: { type: "string" }, new_path: { type: "string" }, expected_content_hash: { type: "string", description: "Optional source doc_content_hash — rename rejected on mismatch." } }, required: ["old_path", "new_title"] }, annotations: { readOnlyHint: false, destructiveHint: true, openWorldHint: false } },
      ...
      { name: "materialize_subgroup", description: "Promote an H3 subgroup inside `## Parent of` to a real intermediate parent doc. Atomically creates the intermediate, replaces H3 with a single bullet pointing at it, rewires each affected child's `## Child of`. Use on lint's `subgroup_materialization_candidate`. Optional `expected_source_content_hash` guards the source doc against stale writes (SPRINT-141b).", inputSchema: { type: "object", additionalProperties: false, properties: { source_path: { type: "string" }, subgroup_heading: { type: "string" }, new_doc_title: { type: "string" }, new_doc_path: { type: "string" }, summary: { type: "string" }, expected_source_content_hash: { type: "string" } }, required: ["source_path", "subgroup_heading"] }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false } },
      { name: "create_child", description: "Atomic create-and-link: writes new doc with canonical scaffold AND adds bullet to parent's `## Parent of`. Use instead of write_doc+patch_section for adding child nodes. Refuses on path/title collision. `gate_on_warnings` hard-blocks on lint codes. Optional `expected_parent_content_hash` guards against stale parent writes (SPRINT-141b).", inputSchema: { type: "object", additionalProperties: false, properties: { parent_path: { type: "string" }, title: { type: "string" }, body: { type: "string", description: "Optional body content appended after the scaffold's ## Notes header." }, summary: { type: "string", description: "Optional blockquote summary. Falls back to a placeholder." }, child_path: { type: "string", description: "Optional override for the new doc's path. Default: <parent_dir>/<sanitized_title>.md." }, gate_on_warnings: { type: "array", items: { type: "string" }, description: "Lint codes to hard-block on. Default []." }, expected_parent_content_hash: { type: "string", description: "Optional parent's doc_content_hash — parent-side write rejected on mismatch." } }, required: ["parent_path", "title"] }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false } },
      { name: "add_association", description: "Atomic two-sided assoc: patches both docs' `## Associated with` (optional shared label). Refuses if pair is hierarchically linked or siblings. Idempotent. Use instead of two patch_section calls. Optional `expected_a_content_hash` / `expected_b_content_hash` guard both sides against stale writes (SPRINT-141b).", inputSchema: { type: "object", additionalProperties: false, properties: { a_path: { type: "string" }, b_path: { type: "string" }, label: { type: "string", description: "Optional shared label appended as ` — <label>` to both bullets." }, gate_on_warnings: { type: "array", items: { type: "string" }, description: "Lint codes to hard-block on (in addition to associate_duplicates_hierarchy and sibling_assoc_redundant which are always hard-gated). Default []." }, expected_a_content_hash: { type: "string", description: "Optional a_path's doc_content_hash — first mismatch short-circuits." }, expected_b_content_hash: { type: "string", description: "Optional b_path's doc_content_hash." } }, required: ["a_path", "b_path"] }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false } },
      ...
      { name: "upload_image", description: "Upload an image (jpg/png/gif/webp/svg) as base64 to the vault. Creates a markdown doc that embeds the image. SVGs auto-rasterise to PNG for social-feed compatibility (pass rasterize: false to skip). Cloud-only.", inputSchema: { type: "object", additionalProperties: false, properties: { image_data: { type: "string", description: "Base64-encoded image bytes." }, media_type: { type: "string", description: "One of: image/jpeg, image/png, image/gif, image/webp, image/svg+xml." }, title: { type: "string", description: "Optional H1 for the created doc (defaults to a timestamp)." }, description: { type: "string", description: "Optional blockquote summary." }, path: { type: "string", description: "Optional vault path for the doc (defaults to images/<slug>.md)." }, rasterize: { type: "boolean", description: "SVG only. When true (default) uploads a PNG sibling and embeds it; false skips." } }, required: ["image_data", "media_type"] }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false } },
      { name: "move_doc", description: "Atomic reparent: rewrites child's `## Child of` + both parents' `## Parent of` in one call. If child has multiple parents, `old_parent_path` must disambiguate. Optional `position` (0-idx) for sibling order. Idempotent. Optional three-side guards `expected_child_content_hash` / `expected_old_parent_content_hash` / `expected_new_parent_content_hash` block writes on any mismatch (SPRINT-141b).", inputSchema: { type: "object", additionalProperties: false, properties: { path: { type: "string", description: "Child doc path to reparent." }, new_parent_path: { type: "string", description: "Path of the new parent doc." }, old_parent_path: { type: "string", description: "Disambiguating old parent when the child has multiple Child of bullets. Required only in that case." }, position: { type: "number", description: "Optional 0-indexed bullet position in the new parent's Parent of. Default: append at end." }, gate_on_warnings: { type: "array", items: { type: "string" }, description: "Lint codes to hard-block on. Default []." }, expected_child_content_hash: { type: "string" }, expected_old_parent_content_hash: { type: "string" }, expected_new_parent_content_hash: { type: "string" } }, required: ["path", "new_parent_path"] }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false } },
      { name: "trash_doc", description: "Flag a doc as trashed without reparenting or rewriting its markdown. The doc's `## Child of` and `## Parent of` stay intact so restore is lossless. State persists in `.emdee/trashed.json` keyed by doc path. The renderer filters trashed docs from non-graveyard views (filter happens server-side in /api/index). Original parent is derived from the doc's first Child of bullet; pass `original_parent_path` to override. Idempotent: re-trashing returns ok with the existing entry. Use `restore_doc` to reverse. Optional `expected_content_hash` guards against stale writes (SPRINT-141b).", inputSchema: { type: "object", additionalProperties: false, properties: { path: { type: "string", description: "Path of the doc to trash." }, original_parent_path: { type: "string", description: "Override the auto-derived restore target. Use when the Child of bullet is ambiguous or unresolvable." }, expected_content_hash: { type: "string", description: "Optional doc_content_hash — trash rejected on mismatch." } }, required: ["path"] }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false } },
      ...
      { name: "create_ticket", description: "SPRINT-173 + SPRINT-185: enqueue a ticket in the cross-project queue used by the CMO/CPO/COO/CEO agents and any user-defined agents. `pillar` picks the owning coarse role (required, backwards-compat); `assigned_agent_id` and `sender_agent_id` are optional opaque slugs (convention `project:role`, e.g. 'whatelz:cmo') for per-agent routing. `type` is a free-form event tag; `payload` is arbitrary JSON. Namespace-scoped to the OAuth caller. Cloud-only.", inputSchema: { type: "object", additionalProperties: false, properties: { pillar: { type: "string", enum: ["cmo", "cpo", "coo", "ceo"] }, type: { type: "string", description: "Free-form event tag identifying the ticket kind." }, payload: { type: "object", description: "Arbitrary JSON signal data. Default {}." }, priority: { type: "string", enum: ["low", "medium", "high"], description: "Default 'medium'." }, assigned_agent_id: { type: "string", description: "SPRINT-185: opaque agent slug this ticket is addressed to (e.g. 'whatelz:cmo'). Max 128 chars." }, sender_agent_id: { type: "string", description: "SPRINT-185: opaque agent slug that filed this ticket. Enables reply routing." } }, required: ["pillar", "type"] }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false } },
      { name: "list_tickets", description: "SPRINT-173 + SPRINT-185: list tickets in the caller's queue, newest first. Filter by `pillar`, `status`, and/or `assigned_agent_id`. Primary inbox shape for an agent: `list_tickets({ assigned_agent_id: 'whatelz:cmo', status: 'open' })`. `limit` capped at 200 (default 50). Returns { ok, count, limit, offset, tickets: [...] }. Cloud-only.", inputSchema: { type: "object", additionalProperties: false, properties: { pillar: { type: "string", enum: ["cmo", "cpo", "coo", "ceo"] }, status: { type: "string", enum: ["open", "in_progress", "done", "blocked"] }, assigned_agent_id: { type: "string", description: "SPRINT-185: filter to tickets addressed to this agent slug." }, limit: { type: "number", description: "Max rows (1–200, default 50)." }, offset: { type: "number", description: "Rows to skip (default 0)." } } }, annotations: { readOnlyHint: true, openWorldHint: false } },
      { name: "update_ticket", description: "SPRINT-173: patch an existing ticket's status / priority / payload. `id` is required; at least one of the three patch fields must be present. Setting `status` to 'done' stamps resolved_at (cleared on any other transition). SPRINT-176: `first_resolved_at` is stamped exactly once by trigger on the FIRST done transition and never overwritten — reopen→reclose preserves it. Namespace-scoped: updates only match tickets owned by the caller. Cloud-only.", inputSchema: { type: "object", additionalProperties: false, properties: { id: { type: "string", description: "UUID returned by create_ticket." }, status: { type: "string", enum: ["open", "in_progress", "done", "blocked"] }, priority: { type: "string", enum: ["low", "medium", "high"] }, payload: { type: "object", description: "Replaces the existing payload wholesale." } }, required: ["id"] }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false } },
    ],
  }));
  ...
  server.setRequestHandler(CallToolRequestSchema, async (req): Promise<CallToolResult> => {
    ...
    switch (name) {
      ...
      case "create_ticket":     return await createTicket(ctx, a) as CallToolResult;
      case "list_tickets":      return await listTickets(ctx, a) as CallToolResult;
      case "update_ticket":     return await updateTicket(ctx, a) as CallToolResult;
