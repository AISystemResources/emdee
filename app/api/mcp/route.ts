...
- Paths starting with "__shared__/<owner_id>/" are docs another user has
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      { name: "update_ticket", description: "SPRINT-173: patch an existing ticket's status / priority / payload. `id` is required; at least one of the three patch fields must be present. Setting `status` to 'done' stamps resolved_at; other transitions clear it. Namespace-scoped: updates only match tickets owned by the caller. Cloud-only.", inputSchema: { type: "object", additionalProperties: false, properties: { id: { type: "string", description: "UUID returned by create_ticket." }, status: { type: "string", enum: ["open", "in_progress", "done", "blocked"] }, priority: { type: "string", enum: ["low", "medium", "high"] }, payload: { type: "object", description: "Replaces the existing payload wholesale." } }, required: ["id"] }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false } },
    ],
  }));
