...
- Paths starting with "__shared__/<owner_id>/" are docs another user has
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      { name: "get_neighbors", description: "Return the doc plus its 1-hop neighborhood (parents / children / associates / siblings / mentioned-in). Response includes `interactive_graph_url` — a pre-rendered Cytoscape.js ego graph on emdee.tech. When the user asks to visualise, explore, or map the doc's relationships, embed the URL in an HTML artifact using a minimal iframe: `<iframe src=\"{interactive_graph_url}\" style=\"width:100%;height:600px;border:0\"></iframe>`. Do NOT regenerate the graph yourself — the URL serves the styled visualisation directly and is signed + time-limited.", inputSchema: { type: "object", properties: { path: { type: "string" } }, required: ["path"] }, annotations: { readOnlyHint: true, openWorldHint: false } },
