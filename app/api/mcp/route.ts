...
import {
  ...
  lintDoc, lintVault, lintVaultAutofix, reconcile, lintOrphans, distillDoc, materializeSubgroup, createChild, addAssociation, getImage, moveDoc, trashDoc, restoreDoc,
...
- Paths starting with "__shared__/<owner_id>/" are docs another user has
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      { name: "get_doc", description: "Returns title + summary + preamble + section headings + `doc_content_hash` (sha256 of raw content, first 16 hex). Each section in `sections` has `{ id, heading, content_hash }` — `id` is a stable short string for patch_section / append_section lookup, `content_hash` is the version guard for patch_section. Pass `full=true` for the body. Pass `expected_content_hash` from a prior get_doc to short-circuit: when matching returns `{ unchanged: true, path, doc_content_hash }` and skips section parsing entirely. Pass `format: \"text\"` for bare markdown (raw file when full=true, else H1 + summary + `##` headings), no JSON envelope — ~3× cheaper for body-heavy reads.", inputSchema: { type: "object", properties: { path: { type: "string" }, full: { type: "boolean", description: "Include the full markdown content. Default false — light envelope only." }, expected_content_hash: { type: "string", description: "Hash from a prior get_doc response. If matches current doc, returns { unchanged: true }." }, format: { type: "string", enum: ["json", "text"], description: "Response shape. Default `json`. `text` = bare markdown, no envelope." } }, required: ["path"] } },
      { name: "read_doc_section", description: "Read one H2 section's body without paying for the whole doc. Returns `{ path, section_id, heading, body, content_hash }`. Either `heading` or `section_id` (from get_doc.sections[].id) must be provided — `section_id` is preferred when available because it's an exact match instead of a fuzzy heading-name lookup. Mismatch returns `section_id_heading_mismatch`. Pass `expected_content_hash` from a prior read to short-circuit: when matching returns `{ unchanged: true, section_id, content_hash }`. Use this instead of get_doc(full=true) when you only need one section.", inputSchema: { type: "object", properties: { path: { type: "string" }, heading: { type: "string" }, section_id: { type: "string", description: "Preferred lookup key from get_doc.sections[].id." }, expected_content_hash: { type: "string", description: "Hash from a prior read. If matches, returns { unchanged: true }." } }, required: ["path"] } },
      ...
      { name: "lint_vault", description: "SPRINT-101: batch-lint the caller's entire vault in one call. Runs every per-doc + cross-doc rule against the in-memory index (no per-doc round trips). Returns { scanned, with_warnings, warnings_total, warnings_by_code, docs: [{path, warnings[]}] } with docs sorted by warning count descending. Pass `prefix` to scope to a subtree, `limit` to cap the returned punch list.", inputSchema: { type: "object", additionalProperties: false, properties: { prefix: { type: "string", description: "Optional path prefix filter." }, limit: { type: "number", description: "Optional max docs in the response punch list. Default: all." } } } },
      { name: "reconcile", description: "SPRINT-108 Fix 3: user-facing repair verb for doc_edges drift. Two modes: pass `path` for per-doc reconcile (deletes all doc_edges rows touching the doc, re-runs syncDocEdges from current Storage content), OR pass `all: true` for a full-namespace rebuild (wipes and rebuilds every doc_edges row from markdown truth via backfillNamespace). Use when sidebar shows orphans, or after batch operations that left the graph inconsistent. Cloud-only. Returns { ok, mode, path?, edges_deleted?, docs_scanned?, edges_written? }.", inputSchema: { type: "object", additionalProperties: false, properties: { path: { type: "string", description: "Doc path to reconcile. Either this or `all` must be set." }, all: { type: "boolean", description: "Full-namespace rebuild. Either this or `path` must be set." } } } },
      { name: "lint_orphans", description: "SPRINT-120: detect docs with no incoming hierarchy edge in doc_edges — the docs that render at sidebar root as orphans. Classifies each: `data_layer_drift` (markdown correct, doc_edges stale — auto-fixable), `markdown_drift` (`## Child of` wiki-link doesn't resolve — needs human), `structural_orphan` (no `## Child of` at all). Pass `fix: true` to auto-repair data-layer drift via per-doc reconcile; markdown-drift and structural cases are reported for human review. Cloud-only. Returns { ok, scanned, total_orphans, by_kind, orphans, fixed?, still_needs_human? }.", inputSchema: { type: "object", additionalProperties: false, properties: { fix: { type: "boolean", description: "Auto-fix data_layer_drift orphans by running per-doc reconcile. Markdown-drift + structural orphans are still reported for human review. Default false (scan only)." } } } },
      { name: "lint_vault_autofix", description: "SPRINT-102 vault hygiene auto-fix. Tier 1: strip redundant `## Associated with` bullets (sibling_assoc_redundant / associate_duplicates_hierarchy). Tier 1.5: demote extra `## Child of` bullets to `## Associated with` (multiple_child_of). Tier 2a: add missing `## Parent of` back-edges for asymmetric_child_edge (safe — no cardinality constraint). Tier 2b: for asymmetric_parent_edge, heuristic pick — if target has 0 parents AND is first claimant, add reciprocal `## Child of`; else remove the false claim from source's `## Parent of`. Fully mechanical + idempotent + no data loss. Defaults to dry-run; pass `dry_run: false` to apply. Returns full plan + applied count + failed list + per-tier counters (bullets_to_remove, parents_to_demote, back_edges_to_add, child_of_reciprocals_to_add, false_parent_claims_to_remove).", inputSchema: { type: "object", additionalProperties: false, properties: { dry_run: { type: "boolean", description: "Default true. Pass false to actually write the fixes." } } } },
      ...
      { name: "write_doc_preview", description: "Preview the diff that write_doc would produce.", inputSchema: { type: "object", additionalProperties: false, properties: { path: { type: "string" }, content: { type: "string" } }, required: ["path", "content"] } },
