...
- Paths starting with "__shared__/<owner_id>/" are docs another user has
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      { name: "get_neighbors", description: "Return the doc plus its 1-hop neighborhood (parents / children / associates / siblings / mentioned-in). Response includes `graph.elements` — a Cytoscape.js-compatible ego graph (focal + 1-hop). When the user asks to visualise, explore, or map the doc's relationships, render `graph.elements` as an interactive HTML artifact using Cytoscape.js.", inputSchema: { type: "object", properties: { path: { type: "string" } }, required: ["path"] }, annotations: { readOnlyHint: true, openWorldHint: false } },
