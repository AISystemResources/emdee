...
- Paths starting with "__shared__/<owner_id>/" are docs another user has
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      { name: "write_doc", description: "Create or overwrite a markdown doc. DESTRUCTIVE — always run write_doc_preview first. Pass `gate_on_warnings: [\"code\", ...]` to hard-block the write when any of those lint codes would fire on the proposed content. Optional `expected_content_hash` guards the overwrite case; create case (doc doesn't exist) is guard-passthrough (SPRINT-141a).", inputSchema: { type: "object", additionalProperties: false, properties: { path: { type: "string" }, content: { type: "string" }, gate_on_warnings: { type: "array", items: { type: "string" }, description: "Lint codes to hard-block on. Default []." }, expected_content_hash: { type: "string", description: "Optional doc_content_hash — overwrite rejected on mismatch." } }, required: ["path", "content"] }, annotations: { readOnlyHint: false, destructiveHint: true, openWorldHint: false } },
