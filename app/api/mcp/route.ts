...
- Paths starting with "__shared__/<owner_id>/" are docs another user has
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      { name: "list_docs", description: "Enumerate every doc as {path, title, summary}. `format:\"compact\"` drops summary (~60% cheaper). `format:\"text\"` is newline-delimited paths only (~5× cheaper).", inputSchema: { type: "object", properties: { format: { type: "string", enum: ["json", "compact", "text"] } } } },
      ...
      { name: "search", description: "Case-insensitive search over titles, summaries, and content. `format:\"compact\"` returns just {path, title} — ~80% cheaper for enumerate-then-drill workflows.", inputSchema: { type: "object", properties: { query: { type: "string" }, limit: { type: "number" }, format: { type: "string", enum: ["json", "compact"] } }, required: ["query"] } },
      ...
      { name: "lint_vault_autofix", description: "Auto-fix vault hygiene: strip redundant assoc bullets, demote extra child_of, add missing back-edges, resolve asymmetric parent claims. Mechanical + idempotent + no data loss. Defaults to dry-run; pass `dry_run:false` to apply.", inputSchema: { type: "object", additionalProperties: false, properties: { dry_run: { type: "boolean", description: "Default true. Pass false to actually write the fixes." } } } },
