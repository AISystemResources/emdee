...
- Paths starting with "__shared__/<owner_id>/" are docs another user has
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      { name: "create_ticket", description: "SPRINT-173: enqueue a ticket in the cross-project queue used by the CMO/CPO/COO/CEO agents. `pillar` picks the owning agent; `type` is a free-form event tag (e.g. 'whatsapp_inbound', 'pr_opened'); `payload` is arbitrary JSON signal data. Namespace-scoped to the OAuth caller. Cloud-only.", inputSchema: { type: "object", additionalProperties: false, properties: { pillar: { type: "string", enum: ["cmo", "cpo", "coo", "ceo"] }, type: { type: "string", description: "Free-form event tag identifying the ticket kind." }, payload: { type: "object", description: "Arbitrary JSON signal data. Default {}." }, priority: { type: "string", enum: ["low", "medium", "high"], description: "Default 'medium'." } }, required: ["pillar", "type"] }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false } },
      { name: "list_tickets", description: "SPRINT-173: list tickets in the caller's queue, newest first. Filter by `pillar` and/or `status`. `limit` capped at 200 (default 50). Returns { ok, count, limit, offset, tickets: [...] }. Cloud-only.", inputSchema: { type: "object", additionalProperties: false, properties: { pillar: { type: "string", enum: ["cmo", "cpo", "coo", "ceo"] }, status: { type: "string", enum: ["open", "in_progress", "done", "blocked"] }, limit: { type: "number", description: "Max rows (1–200, default 50)." }, offset: { type: "number", description: "Rows to skip (default 0)." } } }, annotations: { readOnlyHint: true, openWorldHint: false } },
