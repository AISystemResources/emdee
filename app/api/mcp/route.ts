...
- Paths starting with "__shared__/<owner_id>/" are docs another user has
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      { name: "lint_vault_autofix", description: "SPRINT-102 Tier 1: mechanically remove `## Associated with` bullets flagged by `sibling_assoc_redundant` or `associate_duplicates_hierarchy` — both codes mean the assoc is semantically empty because the relationship already exists via hierarchy or shared parent. Defaults to dry-run (returns the plan without writing); pass `dry_run: false` to actually apply. Returns { tier, dry_run, scanned, planned_fixes, docs_to_modify, bullets_to_remove, applied, failed?, remaining_warnings_estimate }. Only Tier 1 codes are shipped; asymmetric-edge auto-repair is deferred.", inputSchema: { type: "object", additionalProperties: false, properties: { dry_run: { type: "boolean", description: "Default true. Pass false to actually write the fixes." } } } },
